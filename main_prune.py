...
def run(pretrained_path, ratio, threshold, weight_replace=False, verbose=True, save_dir='./result/prune/'):
    """
    Pruning pipeline
    ...
    modnet.load_state_dict({k.replace('module.', ''): v for k, v in torch.load(pretrained_path).items()}, strict=False)
    ...
    # Save pruned modnet
    save_path = f'{save_dir}modnet_p_new_trimap_0027_lr0.0001_ratio_{ratio}_thresh_{threshold}.ckpt'
    ...
    prune_info_path = f'{save_dir}modnet_p_new_trimap_0027_lr0.0001_ratio_{ratio}_thresh_{threshold}.json'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_path', type=str, default='./pretrained/new_mobilenetv2_human_seg.ckpt', required=False, 
