...
viz = Visdom()
logging.basicConfig(filename='./my_log/m_p_n_t_0027_lr0.0001_r_0.5_t_0.5_train.log', level=logging.INFO, datefmt='%Y-%m-%d %H:%M:%S')
logging.info('------------------------------------')
...
if __name__ == '__main__':
    ...
    viz.line([0.], [0], win='semantic_loss', opts=dict(title='Semantic Loss', legend=['semantic_loss']))
    viz.line([0.], [0], win='detail_loss', opts=dict(title='Detail Loss', legend=['detail_loss']))
    viz.line([0.], [0], win='matte_loss', opts=dict(title='Matte Loss', legend=['matte_loss']))


    # define cmd arguments
    parser = argparse.ArgumentParser()
    parser.add_argument('--model-path', type=str, default='./result/prune/new_mobilenetv2_human_seg_r0.5_t0.5/modnet_p_new_trimap_0027_lr0.0001_ratio_0.5_thresh_0.5.ckpt', required=False, 
    ...
    for epoch in range(EPOCHS):
        ...
        for idx, (image, trimap, gt_matte) in enumerate(dataloader):
            ...
            # Record losses
            semantic_losses.append(float(semantic_loss))
            detail_losses.append(float(detail_loss))
            matte_losses.append(float(matte_loss))

            print(f'Batch {(idx + 1) * BS}/{len(mattingDataset)} - '
                  f'semantic_loss: {semantic_loss:.4f}, '
                  f'detail_loss: {detail_loss:.4f}, '
                  f'matte_loss: {matte_loss:.4f}\r', end='')

        # Calculate average losses for the epoch
        avg_semantic = float(np.mean(semantic_losses))
        avg_detail = float(np.mean(detail_losses))
        avg_matte = float(np.mean(matte_losses))

        all_semantic_losses.append(avg_semantic)
        all_detail_losses.append(avg_detail)
        all_matte_losses.append(avg_matte)
        ...
        # Log the results
        info = f"Epoch: {epoch + 1}/{EPOCHS} - Semantic Loss: {avg_semantic:.4f}, Detail Loss: {avg_detail:.4f}, Matte Loss: {avg_matte:.4f}, LR: {optimizer.param_groups[0]['lr']}"
        print(info)
        logging.info(info)
        lr_scheduler.step()
